...
2. 保持逻辑名不变，例如 `MOTOR_AIN1`、`TRACK_CH1`、`LCD_SPI`。这样 `Bsp/board_pins.h` 和上层代码通常不用改。
...
- `DEBUG_UART`：UART0，PA28/PA31，115200，RX 中断。
- `IMU_UART`：UART2，PA21/PA22，115200，RX 中断。
- `LCD_SPI`：SPI1，PB9/PB8，PICO-only，4 MHz，CPOL=1、CPHA=1。
- `SYSTICK`：1 ms 系统节拍。

PA2 不允许出现任何 `pin.$assign = "PA2"`。当前配置不占用 I2C，也不配置未来的 UART3。
