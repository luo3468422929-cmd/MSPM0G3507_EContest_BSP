...
{
    g_uart[UART_ID_DEBUG].instance = UART_DEBUG_INST;
    g_uart[UART_ID_IMU].instance = UART_IMU_INST;
    ...
    NVIC_ClearPendingIRQ(UART_DEBUG_IRQn);
    NVIC_EnableIRQ(UART_DEBUG_IRQn);
    NVIC_ClearPendingIRQ(UART_IMU_IRQn);
    NVIC_EnableIRQ(UART_IMU_IRQn);
    return STATUS_OK;
}
...
{
    if (DL_UART_getPendingInterrupt(UART_IMU_INST) == DL_UART_IIDX_RX) {
