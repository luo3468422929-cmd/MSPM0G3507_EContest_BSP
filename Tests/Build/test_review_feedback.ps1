...
Assert-FileMatches 'Control\scheduler.h' 'SCHEDULER_TASK_200_MS' `
    'Scheduler must expose the 200 ms display task'
Assert-FileMatches 'User\task.c' 'Scheduler_IsDue\(SCHEDULER_TASK_200_MS' `
    'LCD status update must run on the 200 ms task'
