/**
 * @file track.h
 * @brief 定义亚博八路 MCU 灰度模块的统一寻迹数据和校准接口。
 *
 * 所属层：Hardware 传感器层。当前适配器从 I2C 地址 0x12、寄存器 0x30
 * 读取数字位图；未来换 12 路模块时应保持 Track_Update/GetData 接口。
 */
#ifndef TRACK_H
#define TRACK_H
...
typedef struct {
    ...
    uint8_t activeMask;  /**< bit0=逻辑最左 X1，bit7=逻辑最右 X8。 */
...
/** @brief 把一帧八路样本纳入最小/最大统计；参数非法时忽略。 */
